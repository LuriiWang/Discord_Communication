...
deal_data.lm <- lmer(dialog_speed~as.logical(active.developer) + as.logical(roles) + lexicons + as.logical(code.snippets) + as.logical(URLs) + as.logical(weekday) + as.logical(daytime) + readbility.CLI. + as.logical(user.mentions) + (1|community_id), deal_data)
